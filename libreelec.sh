...
if [ ! -f $daemonconfigfile ]; then
	...
	echo '# Desk Pi Fan Configuration' >> $daemonconfigfile
	echo '#' >> $daemonconfigfile
...
deskpi_create_file $shutdownscript
...
deskpi_create_file $powerbuttonscript
...
echo "Description=DeskPi One Fan and Button Service" >> $daemonfanservice
...
deskpi_create_file $removescript

# Uninstall Script
echo '#!/bin/bash' >> $removescript
echo 'echo "-------------------------"' >> $removescript
echo 'echo "Desk Pi Uninstall Tool"' >> $removescript
echo 'echo "-------------------------"' >> $removescript
...
echo 'echo "# DeskPi One Fan Speed Configuration" >> $daemonconfigfile' >> $configscript
echo 'echo "#" >> $daemonconfigfile' >> $configscript
...
needsshutdownedit=0
if [ -f $deskpishutdownscript ]; then
	needsshutdownedit=1
	echo '# Kindly add the following lines under the "halt" and "poweroff" (remove "#" from the start)' >> $deskpishutdownscript
	echo '#    /usr/bin/python '$shutdownscript >> $deskpishutdownscript
else
	deskpi_create_file $deskpishutdownscript
	# Shutdown Script
	echo '#!/bin/bash' >> $deskpishutdownscript
	echo 'case "$1" in' >> $deskpishutdownscript
	echo '  halt)' >> $deskpishutdownscript
	echo '    /usr/bin/python '$shutdownscript >> $deskpishutdownscript
	echo '    ;;' >> $deskpishutdownscript
	echo '  poweroff)' >> $deskpishutdownscript
	echo '    /usr/bin/python '$shutdownscript >> $deskpishutdownscript
	echo '    ;;' >> $deskpishutdownscript
	echo '  reboot)' >> $deskpishutdownscript
	echo '    #' >> deskpieshutdownscript
	echo '    ;;' >> $deskpishutdownscript
	echo '  *)' >> $deskpishutdownscript
	echo '    # your commands here' >> $deskpishutdownscript
	echo '    ;;' >> $deskpishutdownscript
	echo 'esac' >> $deskpishutdownscript

	chmod 755 $deskpishutdownscript
fi
...
echo "***************************"
echo "DeskPi One Setup Completed."
echo "***************************"
